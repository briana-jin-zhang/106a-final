...
class MujocoModel(object):
    ...
    def correct_naming(self, names):
        ...
        if type(names) is str:
            ...
            return [self.naming_prefix + name if not self.exclude_from_prefixing(name) else name for name in names if name is not None]
...
class MujocoXMLModel(MujocoXML, MujocoModel):
    ...
    def __init__(self, fname, idn=0):
        ...
        def _add_default_name_filter(element, parent):
            ...
            if element.tag == "geom" and element.get("name") is None and element.get("geom") is None:
                group = group_mapping[element.get("group")]
                print(f"g{ctr_mapping[group]}_{group}")
